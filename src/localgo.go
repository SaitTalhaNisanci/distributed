...
func (kv *ShardKV) receive_shard(args *SendShardArgs) Result {
    ...
    fmt.Println("RECEIVE SHARD : ",kv.me," ",kv.is_behind(args.Config_num))
    _,found := kv.cache[combine(args.Config_num,args.Shard_index)]
    if found {
				reply.Err = OK
				return reply 	
		}
...
func (kv *ShardKV) Receive_shard(args *SendShardArgs,reply *SendShardReply) error {
  ...
  kv.mu.Unlock()
  for _,k :=range args.Storage {
   if _,ok := kv.muSeq[k.Key] ; !ok {
   			kv.muSeq[k.Key] = &sync.Mutex{}
   }
   kv.muSeq[k.Key].Lock()
   defer kv.muSeq[k.Key].Unlock()
	}
  op := Op{"ReceiveShard",nrand(),*args}
  kv.propose(op)
   kv.evaluate()
  internalReply := kv.id2res[op.OpId]
...
func(kv *ShardKV) put_append(args *PutAppendArgs) Result {
   ...
   if kv.check_duplicates(args.OpId){
      fmt.Println("for key : ", args.Key, " ",args.Value)
			reply.Err =OK
			return reply
		}
   //Case 1: We have the shard  
   fmt.Println("PUTAPPEND1 : " ,args.Key, "  ", args.Value)
   if kv.has_shard(args.Key) && kv.currently_has_shard(args.Key) {
   fmt.Println("PUTAPPEND2 : " ,args.Key, "  ", args.Value)
...
func StartServer(gid int64, shardmasters []string,
  ...
  kv.cache = map[string] bool{}
  kv.id2res = map[int64] Result{}
	rpcs := rpc.NewServer()
	rpcs.Register(kv)
