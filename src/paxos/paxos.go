...
func (px *Paxos) broadcast_prepare(seq int, propose_num int) (int, bool, interface{}, int) {
	...
	for _, peer := range px.peers {
		if peer == px.peers[px.me] {
			...
			if ok {
				px.done[peer] = max(px.done[peer], reply.Highest_done) // Over RPC we learned the current peer's highest done update it in this peer's done map
				px.free_state(px.Min())                                // Try to free this peers state
				//Update the highest accepted proposal
...
func (px *Paxos) broadcast_accept(seq int, propose_num int, v interface{}) bool {
	...
	for _, peer := range px.peers {
		...
		if peer == px.peers[px.me] {
			...
			if ok {
				px.done[peer] = max(px.done[peer], reply.Highest_done) //update done entry
				px.free_state(px.Min())                                // Since we might have updated done entry for this peer it is possible that we need to free some states.
				if reply.OK {
					total_ok++
				}
			}
